...
void batch() {
    ...
    for (int i = 0; i < n_dim; i ++) {
        auto v = *paths[i];
        if (v.lock) babies.erase(v._id);
        if (v.baby) {
            if (v.lock && v.baby -> lock) {
                parents.erase(v.baby -> _id);
                parents.erase(v._id);
                parents.erase(v.baby -> _id);
                babies.erase(v._id);
                babies.erase(v.baby -> _id);
            } else
                v.baby = 0;
        }
    ...
    for (int i = 0; i < n_dim; i ++)
        if (std::get<0>(result[i]) < (int) std::numeric_limits<int>::max()) {
            ...
            if (parents.find(paths[v_idx] -> _id) != parents.end() && babies.find(paths[u_idx] -> _id) != babies.end()) {
                ...
                v.baby = &u;
                std::cout << "- " << v.baby << ", " << v._id << std::endl;
                v.cross = cross;
            }
        }
    for (auto const &it : stds) {
        auto std = it.second;
        std::cout << "+ " << std.baby << ", " << std._id << std::endl;
        if (std.baby)
            std::cout << std._id << " -> " << std.baby -> _id << std::endl;
    }
}

void match(int undo, int redo) {
    std::cout << "lets match" << std::endl;
    // first dones_q
    // here first sells_q
    ...
    while (redo && already.size() < orphans.size()) {
        ...
        for (auto const &head : heads) {
            ...
            for (auto const &orphan_name : orphans) {
                ...
                auto delay = head.eta;
                if (head.baby) delay = head.baby -> eta;
...
Std sell(std::string _id, int length, std::string args[]) {  // no need for anything with topology we can find if it is batch and who is, KHAFE baba
    auto std = stds[args[0]];
    std.lock = true;
    if (length == 2) {
        auto baby = stds[args[1]];
        std.baby = &baby;
        baby.lock = true;
        std.cross = args[2] == "x" ? true : false;
    }
    sells_q.push(make_tuple(_id, std));
